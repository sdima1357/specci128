...
//	taskENTER_CRITICAL();
	...
	{
		mprintf("res==FR_NO_FILESYSTEM\r\n");
		//~ LCD_Draw_Text("format?",10,130, GREEN, 2, BLACK);
	}
	else if(res==FR_NO_PATH)
	{
		mprintf("res==FR_NO_PATH\r\n");
		//~ LCD_Draw_Text("format?",10,130, GREEN, 2, BLACK);
	}
	else
	{
...
int file_dispatch(struct SYS_EVENT* ev)
{
	...
	if(ev->message==MESS_OPEN)
	{
		...
		mprintf("Class: 		%d\r\n",CardInfo.Class);
		mprintf("CardType: 	%d\r\n",CardInfo.CardType);
		mprintf("CardVersion: 	%d\r\n",CardInfo.CardVersion);
		mprintf("BlockNbr: 	%d\r\n",CardInfo.BlockNbr);
		mprintf("BlockSize:	%d\r\n",CardInfo.BlockSize);
		mprintf("LogBlockNbr:	%d\r\n",CardInfo.LogBlockNbr);
		mprintf("LogBlockSize:	%d\r\n",CardInfo.LogBlockSize);
		mprintf("RelCardAdd:	%d\r\n",CardInfo.RelCardAdd);
		rfilesA = 0;
		f_mount(NULL,SDPath, 0);
		if(f_mount(&SDFatFS,SDPath, 0) != FR_OK)
		{
			mprintf("f_mount Error!\r\n");
	...
	else if(ev->message==MESS_REPAINT)
	{
		...
		else
		{
			int k;
			//LCD_fillRect(0,0,LCD_getWidth(),LCD_getHeight(),BLACK);
			mprintf("paint_ rfilesA %d  selection %d \r\n",rfilesA,selection);
			...
			for(k = 0;k <NUmL;k++)
			{
				...
				memset(buffer,' ',0x20);
				char * type[] = {"0:","T:","Z:","M:"};
				LCD_Draw_Text("  ",xScr-16,yScr+(k)*8, BLACK, 1, BLACK);
				if(n<rfilesA)
				{
					...
					LCD_Draw_Text(type[strListArray[n][0]],xScr-16,yScr+(k)*8, GREEN, 1, (selection==n)?RED:BLACK);
				}
			...
			mprintf("paint_ok rfilesA %d  selection %d \r\n",rfilesA,selection);
			return 1;
		}
	}
