...
MP3DecInfo *AllocateBuffers(void)
{
	 ...
	 */
	uint8_t *pnt =  getRamBuffer0x4000(1);
...
//	static SubbandInfo s_sbi;
	...
	fh = pnt;pnt+=sizeof(FrameHeader);//&s_fh;

	si = pnt;pnt+=sizeof(SideInfo);//&s_si;
	sfi =pnt;pnt+=sizeof(ScaleFactorInfo);// &s_sfi;
	hi = pnt;pnt+=sizeof(HuffmanInfo);//&s_hi;
	di = pnt;pnt+=sizeof(DequantInfo);//&s_di;
	mi = pnt;pnt+=sizeof(IMDCTInfo);//&s_mi;
	sbi = pnt;pnt+=sizeof(SubbandInfo);//&s_sbi;
	mprintf("diff = %d\r\n",pnt-getRamBuffer0x4000(2));
	HAL_Delay(2000);
